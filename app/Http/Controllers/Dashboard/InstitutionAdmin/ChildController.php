...
class ChildController extends Controller
{
    ...
    public function update(Request $request, Child $child): RedirectResponse
    {
        ...
        $newGuardianData = $this->validateNewGuardianForEdit($request, $canManageBilling, count($linkedGuardianIds));
        ...
        DB::transaction(function () use (
            ...
            if ($newGuardianData !== null) {
                $this->attachNewGuardianForEdit(
                    $child,
                    $newGuardianData,
                    $institution->id,
                    $canManageBilling,
    ...
    private function validateChild(Request $request, int $institutionId, ?Child $child = null): array
    {
        ...
        // Megjegyzés: az "is_eater" mezőt SZÁNDÉKOSAN nem validáljuk itt.
        // Ez a metódus a store()-ban ÉS az update()-ben is fut, de az
        // étkezési állapot (is_eater) mezője csak a gyermek felvitelekor
        // (create.blade.php) jelenik meg - a szerkesztő űrlapon (edit.blade.php)
        // az étkezési beállítások már egy külön oldalra kerültek
        // (ld. "Étkezési beállítások" link, ChildMealSettingController). A
        // create()-only is_eater ellenőrzést a validateInitialMealSetting()
        // végzi, ami csak a store()-ból hívódik.
    }
    ...
    private function attachGuardian(
        ...
        if ($guardianAttachment['guardian_mode'] === 'existing') {
            ...
            if (empty($guardianAttachment['billing_enabled'])) {
                $this->ensureDefaultBillingProfile($child, $guardian, $institutionId, $canManageBilling);
            }
        ...
        if ($canManageBilling && ! empty($guardianAttachment['billing_enabled'])) {
            ...
            // Az admin nem jelölte be kifejezetten a számlázást ennél az új
            // gondviselőnél - mivel ő az első (és ilyenkor egyetlen)
            // gondviselő, akit a gyermekhez kapcsolunk, automatikusan őt
            // állítjuk be alapértelmezett számlafogadónak, hogy a gyermeknek
            // sose maradjon számlázási adat nélkül. Ez csak egy kényelmi
            // alapérték: a Számlázási címek menüpontban bármikor
            // módosítható vagy másik gondviselőre/fizetőre cserélhető.
            $this->ensureDefaultBillingProfile($child, $guardian, $institutionId, $canManageBilling);
        }
    }

    /**
     * Ha a gyermeknek jelenleg nincs érvényes elsődleges számlázási
     * profilja, és az admin ebben a kérésben nem állított be kifejezetten
     * egyet, akkor a most hozzákapcsolt (mindig az első) gondviselőt
     * alapértelmezetten beállítjuk számlafogadónak - a saját nevével,
     * "Szülő / gondviselő" fizetőtípussal. Ha a gondviselőnek már van
     * (más gyermekhez felvett) aktív számlázási profilja, azt használjuk
     * fel újra, nem hozunk létre duplikátumot.
     *
     * Ez csak egy kényelmi alapérték: az admin a Számlázási címek
     * menüpontban (ParentController@edit/update) bármikor módosíthatja
     * vagy másik gondviselőre/fizetőre cserélheti - az a funkció
     * változatlanul megmarad.
     */
    private function ensureDefaultBillingProfile(
        Child $child,
        Guardian $guardian,
        int $institutionId,
        bool $canManageBilling
    ...
    /**
     * A szerkesztő oldalon (edit.blade.php) egy "+ Új gondviselő
     * hozzáadása" blokk teszi lehetővé, hogy a meglévő (max. 2) gondviselő
     * mellé egy harmadikat NE, de egy másodikat felvegyünk - ugyanazokkal
     * a mezőkkel, mint amiket a meglévő gondviselők szerkesztésénél
     * (validateGuardianEdits/applyGuardianEdits) amúgy is használunk.
     * Nincs külön "hozzáadás" jelölőnégyzet: azt tekintjük hozzáadási
     * szándéknak, ha az űrlap "new_guardian" blokkjában a vezetéknév vagy
     * a keresztnév ki van töltve - így a be nem nyitott/üres blokk
     * egyszerűen figyelmen kívül marad, nem dob hamis "kötelező mező"
     * hibát minden mentésnél.
     */
    private function validateNewGuardianForEdit(Request $request, bool $canManageBilling, int $currentGuardianCount): ?array
    {
        $hasNewGuardianInput = $request->filled('new_guardian.last_name') || $request->filled('new_guardian.first_name');

        if (! $hasNewGuardianInput) {
            return null;
        }

        if ($currentGuardianCount >= 2) {
            throw ValidationException::withMessages([
                'new_guardian.last_name' => 'Egy gyermekhez legfeljebb 2 gondviselő kapcsolható.',
            ]);
        }

        $validated = $request->validate([
            'new_guardian.prefix' => ['nullable', 'string', 'max:30'],
            'new_guardian.last_name' => ['required', 'string', 'max:100'],
            'new_guardian.first_name' => ['required', 'string', 'max:100'],
            'new_guardian.email' => ['nullable', 'email', 'max:191'],
            'new_guardian.phone' => ['nullable', 'string', 'max:50'],
            'new_guardian.bank_account_holder' => [$canManageBilling ? 'nullable' : 'prohibited', 'string', 'max:200'],
            'new_guardian.bank_account_number' => [$canManageBilling ? 'nullable' : 'prohibited', 'string', 'max:64'],
            'new_guardian.address_type' => ['nullable', 'string', 'max:50'],
            'new_guardian.country' => ['nullable', 'string', 'max:100'],
            'new_guardian.postal_code' => ['nullable', 'string', 'max:10'],
            'new_guardian.city' => ['nullable', 'string', 'max:100'],
            'new_guardian.street_name' => ['nullable', 'string', 'max:255'],
            'new_guardian.street_type' => ['nullable', 'string', 'max:50'],
            'new_guardian.house_number' => ['nullable', 'string', 'max:30'],
            'new_guardian.floor' => ['nullable', 'string', 'max:20'],
            'new_guardian.door' => ['nullable', 'string', 'max:20'],
            'new_guardian.active' => ['nullable', 'boolean'],
            'new_guardian.relationship_type' => ['nullable', Rule::in(Guardian::RELATIONSHIP_TYPES)],
            'new_guardian.is_legal_representative' => ['nullable', 'boolean'],
            'new_guardian.has_no_custody' => ['nullable', 'boolean'],
            'new_guardian.is_emergency_contact' => ['nullable', 'boolean'],
            'new_guardian.receives_family_allowance' => ['nullable', 'boolean'],
        ], [
            'new_guardian.last_name.required' => 'Az új gondviselő vezetékneve kötelező.',
            'new_guardian.first_name.required' => 'Az új gondviselő keresztneve kötelező.',
        ]);

        return $validated['new_guardian'];
    }

    /**
     * Új Guardian rekord létrehozása és a gyermekhez kapcsolása a
     * szerkesztő oldal "+ Új gondviselő hozzáadása" blokkjából - ugyanaz
     * a mezőkészlet és ugyanaz a "üres string -> null" konvenció, mint az
     * applyGuardianEdits()-ben a meglévő gondviselőknél. Ez a blokk maga
     * nem kínál külön számlázási mezőket, de ha a gyermeknek még nincs
     * érvényes elsődleges számlázási profilja (pl. mert eddig egyáltalán
     * nem volt gondviselője), az itt felvitt (első) gondviselőt
     * ensureDefaultBillingProfile() automatikusan beállítja alapértelmezett
     * számlafogadónak - ez a Számlázási címek menüpontban továbbra is
     * bármikor módosítható vagy másik gondviselőre/fizetőre cserélhető.
     */
    private function attachNewGuardianForEdit(
        Child $child,
        array $data,
        int $institutionId,
        bool $canManageBilling,
