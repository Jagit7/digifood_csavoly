...
class UpdateParentBillingProfileRequest extends FormRequest
{
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator): void {
            if ($this->boolean('billing_same_as_address')) {
                ...
                $postalCode = trim((string) $guardian?->postal_code);
                $city = trim((string) $guardian?->city);
                $address = trim(implode(' ', array_filter([
                    trim((string) $guardian?->street_name),
                    trim((string) $guardian?->street_type),
                    trim((string) $guardian?->house_number),
                    trim((string) $guardian?->floor),
                    trim((string) $guardian?->door),
                ])));
    ...
    private function primaryGuardian(): ?Guardian
    {
