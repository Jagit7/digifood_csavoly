...
@section('page_title', 'Számlák')

@push('styles')
    <style>
        .df-parent-invoices-page .df-invoice-card {
            border: 0;
            border-radius: 1.5rem;
            box-shadow: 0 20px 48px rgba(15, 23, 42, 0.08);
        }

        .df-parent-invoices-page .df-invoice-info-bar {
            border-radius: 1rem;
        ...
        .df-parent-invoices-page .df-invoice-detail-grid {
            display: grid;
            grid-template-columns: repeat(auto-fit, minmax(180px, 1fr));
            gap: 1rem;
        }

        .df-parent-invoices-page .df-invoice-detail-item {
            border-radius: 1rem;
        ...
        .df-parent-invoices-page .df-invoice-label {
            font-size: 0.78rem;
            font-weight: 700;
        ...
        .df-parent-invoices-page .df-invoice-value {
            font-size: 1rem;
            font-weight: 600;
            color: #111827;
        }

        .df-parent-invoices-page .table > :not(caption) > * > * {
            vertical-align: middle;
        }

        @media (max-width: 767.98px) {
            .df-parent-invoices-page .df-invoice-actions {
                min-width: 12rem;
            }
        }
    </style>
...
@section('content')
    <div class="df-parent-invoices-page">
            ...
            <div class="card-body p-4">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
                    <div>
                        ...
                        <div class="text-muted small">A bejelentkezett szülőhöz tartozó számlák listája.</div>
                    </div>

                    <form method="GET" action="{{ route('parent.invoices') }}" class="d-flex align-items-center gap-2">
                        ...
                        <table class="table align-middle mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-end">Bruttó összeg</th>
                                    <th>Fizetési mód</th>
                                    <th>Státusz</th>
                            ...
                            <tbody>
                                @foreach($invoices as $row)
                                    <tr>
                                        ...
                                        <td>{{ $row['fulfillment_date_label'] }}</td>
                                        <td>{{ $row['due_date_label'] }}</td>
                                        <td>{{ $row['period_label'] }}</td>
                                        <td title="{{ $row['child_names']->join(', ') }}">{{ $row['child_summary'] }}</td>
                                        <td class="text-end">{{ $row['gross_amount_label'] }}</td>
                                        ...
                                        <td colspan="10" class="bg-light">
                                            <div class="df-invoice-detail-grid p-2">
                                                ...
                                                <div class="df-invoice-detail-item">
                                                    ...
                                                    <div class="df-invoice-value">{{ $row['details']['issue_date_label'] }}</div>
                                                </div>
                                                <div class="df-invoice-detail-item">
                                                    <div class="df-invoice-label mb-1">Teljesítés dátuma</div>
                                                    <div class="df-invoice-value">{{ $row['details']['fulfillment_date_label'] }}</div>
                                                </div>
                                                <div class="df-invoice-detail-item">
                                                ...
                                                <div class="df-invoice-detail-item">
                                                    <div class="df-invoice-label mb-1">Fizetési mód</div>
                                                    <div class="df-invoice-value">{{ $row['details']['payment_method_label'] }}</div>
                                                </div>
                                                <div class="df-invoice-detail-item">
                                                    <div class="df-invoice-label mb-1">Számlázási név</div>
                                                    <div class="df-invoice-value">{{ $row['details']['billing_name'] }}</div>
                                                </div>
                                                <div class="df-invoice-detail-item">
                                                    <div class="df-invoice-label mb-1">Számlázási cím</div>
                                                    <div class="df-invoice-value">{{ $row['details']['billing_address'] }}</div>
                                                </div>
                                                <div class="df-invoice-detail-item">
                                                    <div class="df-invoice-label mb-1">Érintett gyermekek</div>
                                                    <div class="df-invoice-value">{{ $row['details']['child_names']->join(', ') }}</div>
                                                </div>
                                                <div class="df-invoice-detail-item">
                                                ...
                                                <div class="df-invoice-detail-item">
                                                    <div class="df-invoice-label mb-1">Nettó összeg</div>
                                                    <div class="df-invoice-value">{{ $row['details']['net_amount_label'] }}</div>
                                                </div>
                                                <div class="df-invoice-detail-item">
                                                    <div class="df-invoice-label mb-1">Áfa</div>
                                                    <div class="df-invoice-value">{{ $row['details']['vat_amount_label'] }}</div>
                                                </div>
                                                <div class="df-invoice-detail-item">
                                                    <div class="df-invoice-label mb-1">Bruttó összeg</div>
                                                    <div class="df-invoice-value">{{ $row['details']['gross_amount_label'] }}</div>
                                                </div>
                                                <div class="df-invoice-detail-item">
                                                    <div class="df-invoice-label mb-1">Szolgáltatói azonosító</div>
                                                    <div class="df-invoice-value">{{ $row['details']['provider_invoice_id'] }}</div>
                                                </div>
                                                <div class="df-invoice-detail-item">
                                                    <div class="df-invoice-label mb-1">Státusz</div>
                                                    <div class="df-invoice-value">
                                                        <span class="badge {{ $row['details']['status_class'] }}">{{ $row['details']['status_label'] }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
